...
if ( ! function_exists( 'newsarc_pagination_style' ) ) :
	/**
	 * Styles the pagination style.
	 *
	 * @see select_pagination_style().
	 */
	function newsarc_pagination_style() {
	$select_pagination_style = newsarc_get_option( 'select_pagination_style');
	...
	<div class="newsarc-nav-pagination">
		...
		newsarc_ajax_pagination( $select_pagination_style );

		elseif ( 'pagination_numeric' == $select_pagination_style ) :

			$prev_text        = sprintf(
				'<span class="nav-prev-text">%s</span>',
				newsarc_get_theme_svg('chevron-left')
			);
			$next_text        = sprintf(
				'<span class="nav-next-text">%s</span>',
				newsarc_get_theme_svg('chevron-right')
			);
			...
			$prev_text = sprintf(
				...
				__( 'Older Articles', 'newsarc' )
			);
			$next_text = sprintf(
				'%s <span class="nav-next-text title">%s</span>',
				'<span class="arrow" aria-hidden="true">&rarr;</span>',
				__( 'Newer Articles', 'newsarc' )
			);
