...
class Newsarc_Social_Menu extends Newsarc_Widget_Base {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'widget_newsarc_social_menu';
		$this->widget_description = __( 'Displays social menu if you have set it.', 'newsarc' );
		$this->widget_id          = 'newsarc_social_menu';
		$this->widget_name        = __( 'NewsArc: Social Menu', 'newsarc' );
	...
	protected function get_widget_settings()
	{
	    return array(
			...
			'color'      => array(
				'type'    => 'select',
				'label'   => __( 'Social Links Color', 'newsarc' ),
				'options' => array(
					'has-brand-color' => __( 'Use Brand Color', 'newsarc' ),
					'has-brand-background' => __( 'Use Brand Background', 'newsarc' ),
				),
			...
			'style'      => array(
				...
				'options' => newsarc_get_social_links_styles(),
				'std'     => 'style_1',
			),
			'show_label' => array(
				'type'  => 'checkbox',
				'label' => __( 'Show Label', 'newsarc' ),
				'std'   => false,
			),
			'column'     => array(
				'type'    => 'select',
				'label'   => __( 'Column', 'newsarc' ),
				'desc'    => __( 'Will only work when label is enabled from above and there is enough space to display the columns.', 'newsarc' ),
				'options' => array(
					'one'   => __( 'One', 'newsarc' ),
					'two'   => __( 'Two', 'newsarc' ),
					'three' => __( 'Three', 'newsarc' ),
				),
				'std'     => 'two',
			),
			'align'      => array(
				'type'    => 'select',
				'label'   => __( 'Alignment', 'newsarc' ),
				'options' => array(
	...
	public function widget( $args, $instance ) {
		...
		do_action( 'newsarc_before_social_menu' );
		...
		<div class="wpi-social-widget is-aligned-<?php echo esc_attr( $wrapper_class ); ?>">
			...
			if ( has_nav_menu( 'social' ) ) {
				...
				esc_html_e( 'Social menu is not set. You need to create menu and assign it to Social Menu on Menu Settings.', 'newsarc' );
			}
			?>
		</div>
		<?php

		do_action( 'newsarc_after_social_menu' );
