...
class Newsarc_Carousel_Slider_Post extends Newsarc_Widget_Base
{
    public function __construct()
    {
        $this->widget_cssclass = 'newsarc-slider-widget';
        $this->widget_description = __("Displays recent posts in a carousel slider with an image", 'newsarc');
        $this->widget_id = 'newsarc_carousel_slider_posts';
        $this->widget_name = __('NewsArc: Carousel Posts', 'newsarc');
    ...
    protected function get_widget_settings()
    {
        return array(
            'title' => array(
                ...
                'std' => __('Carousel Posts', 'newsarc'),
            ),
            'display_layout' => array(
                'type' => 'select',
                'label' => __('Display Layout', 'newsarc'),
                'options' => array(
                    'display-regular-layout' => __('Wrapper Layout', 'newsarc'),
                    'display-fullwidth-layout' => __('Full Width Layout', 'newsarc'),
                ),
                'std' => 'display-regular-layout',
                'desc' => __('Works exclusively in the fullwidth widget area', 'newsarc'),
            ),
            ...
            'display_style' => array(
                ...
                'options' => array(
                    'wpi-post-regular' => __('Regular View', 'newsarc'),
                    'wpi-slides-card' => __('Card View', 'newsarc'),
                ),
                'std' => 'wpi-post-regular',
            ),
            'category' => array(
                'type' => 'dropdown-taxonomies',
                'label' => __('Select Category', 'newsarc'),
                'args' => array(
                    'taxonomy' => 'category',
                    'class' => 'widefat',
                    'hierarchical' => true,
                    'show_count' => 1,
                    'show_option_all' => __('&mdash; Select &mdash;', 'newsarc'),
                ),
            ),
            'number' => array(
                'type' => 'number',
                ...
                'min' => 1,
                'std' => 7,
                'label' => __('Total Number of posts to slider', 'newsarc'),
            ),
            'number_of_slides' => array(
                ...
                'max' => 5,
                'std' => 3,
                'label' => __('Number of Slides to show', 'newsarc'),
            ),
            'offset' => array(
                'type' => 'number',
                'step' => 1,
                ...
                'max' => '',
                'std' => '',
                'label' => __('Offset', 'newsarc'),
                'desc' => __('Offsets are used to skip a certain number of WordPress posts before starting output. Set it to 0 if you do not wish to use this feature.', 'newsarc'),
            ),
            'font_size' => array(
                'type' => 'select',
            ...
            'image_size' => array(
                ...
                'options' => array(
                    'medium' => __('Medium', 'newsarc'),
                    'medium_large' => __('Medium Large', 'newsarc'),
                    'large' => __('Large', 'newsarc'),
                    'full' => __('Full', 'newsarc'),
                ),
                'std' => 'large',
            ...
            'show_overlay' => array(
                'type' => 'checkbox',
                'label' => __('Enable background overlay', 'newsarc'),
                'std' => true,
            ),
            'image_overlay_style' => array(
                'type' => 'select',
                'label' => __('Background overlay style', 'newsarc'),
                'options' => array(
                    'regular' => __('Regular', 'newsarc'),
                    'gradient' => __('Gradient', 'newsarc'),
                ),
                'std' => 'gradient',
            ),
            'height' => array(
                ...
                'desc' => __('Change Height of Slides.', 'newsarc'),
            ),
            'arrows' => array(
                'type' => 'checkbox',
            ...
            'autoplay' => array(
                'type' => 'checkbox',
                'label' => __('Autoplay', 'newsarc'),
                'std' => true,
            ),
            'centered' => array(
                'type' => 'checkbox',
                'label' => __('Centered Mode', 'newsarc'),
                'std' => false,
            ),
            'spacebetween' => array(
                'type' => 'number',
                ...
                'max' => '40',
                'std' => 20,
                'label' => __('Space Between', 'newsarc'),
                'desc' => __('Distance between slides in px.', 'newsarc'),
            ),

            'orderby' => array(
                'type' => 'select',
                'std' => 'date',
                'label' => __('Order by', 'newsarc'),
                'options' => array(
            ...
            'show_date' => array(
                'type' => 'checkbox',
                'label' => __('Show Date', 'newsarc'),
                'std' => true,
            ),
            'date_format' => array(
                'type' => 'select',
                'label' => __('Date Format', 'newsarc'),
                'options' => array(
                    'format_1' => __('Format 1', 'newsarc'),
                    'format_2' => __('Format 2', 'newsarc'),
                ),
                'std' => 'format_1',
            ),
            'show_author' => array(
                'type' => 'checkbox',
                'label' => __('Show Author', 'newsarc'),
                'std' => true,
            ),
            'display_author_option' => array(
                'type' => 'select',
                'label' => __('Author Option', 'newsarc'),
                'options' => array(
            ...
            'author_text' => array(
                'type' => 'text',
                'label' => __('Author Text', 'newsarc'),
                'std' => __('By:', 'newsarc'),
                'desc' => __('This only works when the "With Label" option is selected under "Author Option"', 'newsarc'),
            ),
            'show_category' => array(
                'type' => 'checkbox',
                'label' => __('Show Category', 'newsarc'),
                'std' => true,
            ),
            'category_text' => array(
                'type' => 'text',
                'label' => __('Category Text', 'newsarc'),
            ),
            'display_category_option' => array(
                'type' => 'select',
                'label' => __('Category Option', 'newsarc'),
                'options' => array(
            ...
            'number_of_cat' => array(
                ...
                'min' => 1,
                'std' => 1,
                'label' => __('Number of Category to show', 'newsarc'),
            ),
            'show_excerpt' => array(
                'type' => 'checkbox',
                'label' => __('Enable Excerpt', 'newsarc'),
                'std' => false,
            ),
        );
    }
    ...
    protected function get_posts($args, $instance)
    {
        ...
        return new WP_Query(apply_filters('newsarc_carousel_slider_posts_query_args', $query_args));
    }
    ...
    public function widget($args, $instance)
    {
        ...
        do_action('newsarc_before_carousel_slider_posts_with_image');
        ...
        <?php
        do_action('newsarc_after_carousel_slider_posts_with_image');
            ...
            <article id="widget-slide-<?php the_ID(); ?>" <?php post_class('wpi-post ' . esc_attr($display_style)); ?>>
                ...
                <div class="entry-details <?php echo $text_alignment; ?>">
                    ...
                    if (!empty($instance['show_category']) && $instance['show_category']) {
                        newsarc_post_category($display_category_option, $category_text, $number_of_cat);
                    }
                    ?>
                    ...
                    <div class="entry-meta-wrapper">
                        <?php if (!empty($instance['show_date']) && $instance['show_date']) : ?>
                            <div class="entry-meta entry-date posted-on">
                                <span class="screen-reader-text"><?php _e('Post Date', 'newsarc'); ?></span>
                                <?php newsarc_the_theme_svg('calendar'); ?>
                                <?php
                                $date_format = !empty($instance['date_format']) ? $instance['date_format'] : 'format_1';
                                if ('format_1' === $date_format) {
                                    echo esc_html(human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ' . __('ago', 'newsarc'));
                                } else {
                        ...
                        <?php if (!empty($instance['show_author']) && $instance['show_author']) : ?>
                                ...
                                <?php
                                if ($show_author) {
                                    newsarc_posted_by($display_author_option, $author_text);
                                }
                                ?>
