...
/**
 * Adds Newsarc_Author_Widget widget.
 */
class Newsarc_Author_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'newsarc_author_widget',
            esc_html__( 'NewsArc: Author Widget', 'newsarc' ),
            array( 'description' => esc_html__( 'Displays author information.', 'newsarc' ) )
        );

        $this->social_networks = apply_filters(
            'newsarc_author_widget_social_networks',
            array(
                'facebook',
                'twitter',
                'linkedin',
    ...
    private function display_social_links( $instance ) {
        ...
        foreach ( $social_networks as $network ) {
            if ( ! empty( $instance[ $network ] ) ) {
                $svg = Newsarc_SVG_Icons::get_social_link_svg( $instance[ $network ] );
                if ( $svg ) {
    ...
    public function form( $instance ) {
        $fields = array(
            'title'               => esc_html__( 'Title:', 'newsarc' ),
            'author_name'         => esc_html__( 'Author Name:', 'newsarc' ),
            'author_pos'          => esc_html__( 'Author Position:', 'newsarc' ),
            'author_desc'         => esc_html__( 'Description:', 'newsarc' ),
            'author_img'          => esc_html__( 'Image:', 'newsarc' ),
            'image_border_radius' => esc_html__( 'Enable Image Border Radius', 'newsarc' ),
            'social_border_radius' => esc_html__( 'Circular Social Icon', 'newsarc' ),
            'social_links_color'  => esc_html__( 'Social Icon Color', 'newsarc' ),
        );
            ...
            case 'author_img': ?>
                ...
                <div class="image-field">
                    ...
                    <p>
                        ...
                        <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'newsarc' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'newsarc' ); ?>">
                            <?php esc_html_e( 'Upload/Add image', 'newsarc' ); ?>
                        </button>
                        <button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'newsarc' ); ?></button>
                    </p>
                </div>
    ...
    private function get_select_options( $field ) {
        switch ( $field ) {
            case 'social_links_color':
                return array(
                    'has-brand-background'  => esc_html__( 'Has Brand Background', 'newsarc' ),
                    'has-default-color'   => esc_html__( 'Theme Color', 'newsarc' ),
                    'has-brand-color'  => esc_html__( 'Has Brand Color', 'newsarc' ),
                );
