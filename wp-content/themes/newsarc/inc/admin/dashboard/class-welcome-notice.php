...
class NewsArc_Welcome_Notice {
    ...
    public function show_welcome_notice() {
        if (!get_option('newsarc_admin_notice_welcome')) {
    ...
    private function get_import_button_html() {
        return sprintf(
            ...
            esc_attr('mailchimp-for-wp, wpinterface-add-ons, one-click-demo-import'),
            esc_attr__('Get Started', 'newsarc'),
            esc_html__('Get started', 'newsarc')
        );
    }

    public function render_welcome_notice() {
        $dismiss_url = wp_nonce_url(
            remove_query_arg(['activated'], add_query_arg('newsarc-hide-notice', 'welcome')),
            'newsarc_hide_notices_nonce',
            '_newsarc_notice_nonce'
        );
        ...
        ?>
        <div id="message" class="notice notice-success newsarc-notice">
            <a class="newsarc-message-close notice-dismiss" href="<?php echo esc_url($dismiss_url); ?>"></a>
            <div class="newsarc-notice-wrapper">
                <div class="newsarc-notice-welcome">
                    <div class="newsarc-notice-left">
                        <div class="newsarc-notice-subheading">
                            <?php printf(esc_html__('Welcome %1$s!', 'newsarc'), $current_user->user_login); ?>
                        </div>
                        <h2 class="newsarc-notice-heading">
                            <?php esc_html_e('Thank you for choosing NewsArc!', 'newsarc'); ?>
                        </h2>
                    </div>
                </div>
                <div class="newsarc-notice-content">
                    <div class="newsarc-notice-item newsarc-notice-template-import">
                        <h3><?php esc_html_e('Recommended Plugins', 'newsarc'); ?></h3>
                        <p><?php esc_html_e('Let\'s start by installing and activating One Click Demo Import, WPInterface Add-ons and MC4WP: Mailchimp for WordPress', 'newsarc'); ?></p>
                        <?php echo $this->get_import_button_html(); ?>
                    </div>
                    <div class="newsarc-notice-item newsarc-notice-theme-dashboard">
                        <h3><?php esc_html_e('Quick Links', 'newsarc'); ?></h3>
                        <p><?php esc_html_e("Check the quick links below for detailed information and to make the most of all the features our theme offers.", 'newsarc'); ?></p>
                        <div class="dashboard-button-group">
                            <a href="<?php echo esc_url(admin_url('themes.php?page=' . $this->theme_slug)); ?>" class="button button-secondary dashboard-button dashboard-button-secondary">
                                <?php esc_html_e('Theme Dashboard', 'newsarc'); ?>
                            </a>
    ...
    public function handle_hide_notice() {
        if (isset($_GET['newsarc-hide-notice']) && isset($_GET['_newsarc_notice_nonce'])) {
            if (!wp_verify_nonce(wp_unslash($_GET['_newsarc_notice_nonce']), 'newsarc_hide_notices_nonce')) {
                wp_die(__('Action failed. Please refresh the page and retry.', 'newsarc'));
            }
            if (!current_user_can('manage_options')) {
                wp_die(__('Cheatin&#8217; huh?', 'newsarc'));
            }
            $hide_notice = sanitize_text_field(wp_unslash($_GET['newsarc-hide-notice']));
            update_option('newsarc_admin_notice_' . $hide_notice, 1);
        }
    }

    public function handle_import_button() {
        check_ajax_referer('newsarc_demo_import_nonce', 'security');
        $state = [];
        ...
        $response = ['redirect' => admin_url('/themes.php?page=newsarc&tab=dashboard')];
