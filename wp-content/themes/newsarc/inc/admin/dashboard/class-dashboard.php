...
class NewsArc_Dashboard
{
     ...
     * @since 1.0.0
     */
    private $theme_url = 'https://wpinterface.com/themes/newsarc/';
    /**
     * Documentation Url
     *
     * @access private
     ...
     * @since 1.0.0
     */
    private $doc_url = 'https://docs.wpinterface.com/newsarc/';
    /**
     * Preview Url
     *
     * @access private
     ...
     * @since 1.0.0
     */
    private $preview_url = 'https://demo.wpinterface.com/newsarc';
    /**
     * Misc Support Url
     *
     * @access private
     * @since 1.0.0
     */
    private $community_url = 'https://wordpress.org/support/theme/newsarc/';
    ...
    public function create_menu()
    {
        ...
        $theme_page_name = sprintf(esc_html__('%s', 'newsarc'), $theme->Name);
        add_theme_page(
            $theme_page_name,
            $theme_page_name,
            'edit_theme_options',
    ...
    {
        // Bail if we're not on a NewsArc screen or page.
        if (empty($_REQUEST['page']) || false === strpos(sanitize_text_field(wp_unslash($_REQUEST['page'])), 'newsarc')) { // phpcs:ignore WordPress.Security.NonceVerification
            return;
        }
        global $wp_filter;
        $ignore_notices = apply_filters('newsarc_ignore_hide_admin_notices', array());
    ...
    public function dashboard_page()
    {
        ...
        $admin_url = admin_url();
        $tabs = apply_filters(
            'newsarc_dashboard_tabs',
            array(
                'dashboard' => array(
                    'name' => esc_html__('Dashboard', 'newsarc'),
                    'callback' => function () {
                        include __DIR__ . '/views/dashboard.php';
                    },
                ),
                'starter-templates' => array(
                    'name' => esc_html__('Starter Templates', 'newsarc'),
                    'callback' => function () {
                            include __DIR__ . '/views/starter-templates.php';
                    },
                ),
                'products' => array(
                    'name' => esc_html__('Our Recommendation', 'newsarc'),
                    'callback' => function () {
                        include __DIR__ . '/views/products.php';
                    },
                ),
                'free-vs-pro' => array(
                    'name' => esc_html__('Free Vs Pro', 'newsarc'),
                    'callback' => function () {
                        include __DIR__ . '/views/free-vs-pro.php';
                    },
                ),
                'help' => array(
                    'name' => esc_html__('Help', 'newsarc'),
                    'callback' => function () {
        ...
        ?>
        <div class="wrap">
            <div class="newsarc-dashboard-area">
                <div class="newsarc-dashboard-header">
                    <a class="newsarc-logo" href="<?php echo esc_url($this->theme_url); ?>"
                       target="_blank">
                        <img class="newsarc-premium-badge"
                             src="<?php echo esc_url(NEWSARC_PARENT_URL . '/inc/admin/dashboard/images/newsarc-logo.webp'); ?>"
                             alt="<?php esc_attr_e('NewsArc', 'newsarc'); ?>">
                        <span class="newsarc-theme-version">
									<?php
                            ...
                            foreach ($tabs as $id => $tab) :
                                ...
                                <li class="menu-item menu-item-<?php echo esc_attr($id); ?>">
                                    <a id="<?php echo esc_attr($id); ?>"
                                       href=<?php echo esc_url("{$admin_url}themes.php?page=newsarc&tab=$id"); ?>><?php echo esc_html($tab['name']); ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </nav>
                    <div class="dashboard-header-extras">
                        <div class="dashboard-button-group">
                            ...
                            <button type="button" id="dashboard-changelog-button" class="button button-secondary dashboard-button dashboard-button-secondary">
                                <?php esc_html_e('Changelog', 'newsarc'); ?>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                ?>
            </div><!--/.newsarc-dashboard-area-->
        </div><!--/.wrap-->
        <div id="newsarc-dashboard-offcanvas" class="newsarc-dashboard-offcanvas">
            <div class="dialog-head">
                <h3><?php esc_html_e('Latest Updates', 'newsarc'); ?></h3>
            ...
            <div class="dialog-content">
                <?php
                $changelog = new NewsArc_Changelog_Parser();
                // Fetch changelog data.
                $changelog_data = $changelog->get_items();

                if ($changelog_data) {
                    // Output the changelog data
                    echo '<div class="newsarc-changelog">';
                    foreach ($changelog_data as $entry) {
                        echo '<div class="newsarc-changelog-list-item">';
                        echo '<div class="newsarc-changelog-list-head">';
                        echo '<h4 class="newsarc-changelog-version">Version: ' . esc_html($entry['version']) . '</h4>';
                        echo '<p class="newsarc-changelog-date">' . esc_html($entry['date']) . '</p>';
                        echo '</div>'; // newsarc-changelog-list-head
                        // Display each change
                        echo '<div class="newsarc-changelog-change">';
                        foreach ($entry['changes'] as $tag => $changes) {
                            echo '<div class="newsarc-changelog-change-item item--' . esc_html(strtolower($tag)) . '">';
                            echo '<span class="newsarc-changelog-change-type">' . esc_html($tag) . '</span>';
                            echo '<div class="newsarc-changelog-change-list">';
                            foreach ($changes as $change) {
                                echo '<p class="newsarc-changelog-change-desc">' . esc_html($change) . '</p>';
                            }
                            echo '</div>'; // newsarc-changelog-change-list
                            echo '</div >'; // newsarc-changelog-change-item
                        }
                        echo '</div>'; // newsarc-changelog-change
                        echo '</div>'; // newsarc-changelog-list-item
                    }
                    echo '</div>'; // newsarc-changelog
                } else {
