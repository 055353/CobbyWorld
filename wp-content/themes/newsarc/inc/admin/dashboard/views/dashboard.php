...
function import_button_html()
{
    ...
    if (file_exists(WP_PLUGIN_DIR . '/mailchimp-for-wp/mailchimp-for-wp.php') && is_plugin_inactive('mailchimp-for-wp/mailchimp-for-wp.php')) {
        $newsarc_btn_texts = __('Activate WpInterface Add-Ons', 'newsarc');
    ...
    elseif (!file_exists(WP_PLUGIN_DIR . '/wpinterface-add-ons/wpinterface-add-ons.php')&& is_plugin_inactive('wpinterface-add-ons/wpinterface-add-ons.php')) {
        $newsarc_btn_texts = __('Install WpInterface Add-Ons', 'newsarc');
        $button_url = $admin_url . 'plugin-install.php?s=wpinterface-add-ons&tab=search&type=term';
    }
    // Default to view starter templates
    else {
        $newsarc_btn_texts = __('View Starter Templates', 'newsarc');
        $button_url = '#'; // Replace with the actual URL for viewing starter templates
    }

    $html = '<a class="btn-get-started" href="' . esc_url($button_url) . '" data-name="' . esc_attr('mailchimp-for-wp') . '" data-slug="' . esc_attr('mailchimp-for-wp') . '" aria-label="' . esc_attr($newsarc_btn_texts) . '">' . esc_html($newsarc_btn_texts) . '</a>';
    return $html;
}

?>
    <div class="newsarc-dashboard-content">
        <div class="dashboard-content-panel content-panel-upper">
            <div class="postbox">
                <div class="inside newsarc-dashboard-welcome">
                    <div class="newsarc-notice-left">
                        <h2 class="newsarc-notice-heading">
                            <?php
                            printf(
                                esc_html__('Start Building with NewsArc!', 'newsarc')
                            );
                            ?>
                        </h2>
                        <p>
                            <?php esc_html_e('Experience effortless magazine-themed website creation with NewsArc! Explore all the settings and features here.', 'newsarc'); ?>
                        </p>
                        ...
                        <div class="dashboard-button-group">
                            <?php if (!$this->pro_status) { ?>
                                <a href="<?php echo esc_url($this->theme_url); ?>#choose-pricing-plan"
                                   ...
                                   target="_blank">
                                    <?php esc_html_e('Upgrade to Pro', 'newsarc'); ?>
                                </a>
                            <?php } ?>
                            <a href="<?php echo esc_url($this->preview_url); ?>"
                               class="button button-secondary dashboard-button dashboard-button-secondary"
                               target="_blank">
                                <?php esc_html_e('Live Preview', 'newsarc'); ?>
                            </a>

                        </div>

                    </div>
                    <div class="newsarc-notice-right">
                        <div class="newsarc-intro-image">
                            <img src="<?php echo esc_url(get_template_directory_uri()); ?>/inc/admin/dashboard/images/theme-layout.webp" alt="NewsArc WordPress Theme">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="dashboard-content-panel content-panel-lower">
            <div class="dashboard-content-left">
                <div class="postbox newsarc-quick-settings">
                    <div class="postbox-header">
                        <h2 class="hndle ui-sortable-handle"><?php esc_html_e('Quick Settings', 'newsarc'); ?></h2>
                        <a href="<?php echo esc_url(' ' . $admin_url . 'customize.php?'); ?>"
                           class="hndle-controller newsarc-link" target="_blank">
                            <?php echo 'Visit Customizer' . $link_icon; ?>
                        </a>
                    </div>
                    <div class="inside newsarc-available-settings">
                        <?php
                        // Array of settings items
                        $settings_items = array(
                            array(
                                'title' => esc_html__('Site Identity', 'newsarc'),
                                'type' => 'section',
                                'id' => 'title_tagline',
                            ),
                            array(
                                'title' => esc_html__('Header Options', 'newsarc'),
                                'type' => 'panel',
                                'id' => 'newsarc_header_panel',
                            ),
                            array(
                                'title' => esc_html__('Footer Copyright Options', 'newsarc'),
                                'type' => 'panel',
                                'id' => 'newsarc_footer_panel',
                            ),
                            array(
                                'title' => esc_html__('Global Colors', 'newsarc'),
                                'type' => 'section',
                                'id' => 'newsarc_global_colors_section',
                            ),
                            array(
                                'title' => esc_html__('Sidebar Options', 'newsarc'),
                                'type' => 'section',
                                'id' => 'newsarc_global_sidebar_section',
                            ),
                            array(
                                'title' => esc_html__('Blog', 'newsarc'),
                                'type' => 'section',
                                'id' => 'newsarc_blog_section',
                            ),
                        );
                        // Loop through the settings items
                        foreach ($settings_items as $item) {
                            ?>
                            <div class="newsarc-features-list newsarc-free-options">
                                <h4><?php echo esc_html($item['title']); ?></h4>
                                <a href="<?php if (isset($item['type'])) {
                                    echo admin_url('customize.php?autofocus[' . $item['type'] . ']=' . $item['id']);
                                } ?>" class="newsarc-link" target="_blank">
                ...
                <div class="postbox newsarc-premium-settings">
                    <div class="postbox-header">
                        <h2 class="hndle ui-sortable-handle"><?php esc_html_e('Premium Features', 'newsarc'); ?></h2>
                        <?php if (!$this->pro_status) { ?>
                            <a href="<?php echo esc_url($this->theme_url); ?>#choose-pricing-plan"
                               class="hndle-controller newsarc-link" target="_blank">
                    ...
                    <div class="inside newsarc-unavailable-settings">
                        ...
                        foreach ($premium_features_items as $item) {
                            ?>
                            <div class="newsarc-features-list newsarc-premium-features">
                                ...
                                <div class="item-content-right">
                                    <img class="newsarc-premium-badge"
                                         src="<?php echo esc_url(NEWSARC_PARENT_URL . '/inc/admin/dashboard/images/badge.webp'); ?>"
                                         alt="<?php esc_attr_e('NewsArc', 'newsarc'); ?>">
                                </div>
                            </div>
                            <?php
                        }
            ...
            <div class="dashboard-content-right">
                <div class="postbox newsarc-postbox-documentation">
                    <div class="postbox-header">
                        <h3 class="hndle ui-sortable-handle">
                            <?php esc_html_e('Documentation', 'newsarc'); ?>
                        </h3>
                    ...
                    <div class="inside">
                        <p>
                            <?php
                            echo esc_html__('Encountering an issue? Our comprehensive and detailed documentation is designed to address your concerns and provide clear, thorough explanations to resolve any confusion.', 'newsarc');
                            ?>
                        </p>
                        <a href="<?php echo esc_url($this->doc_url); ?>"
                           target="_blank"><?php esc_html_e('Documentation', 'newsarc'); ?></a>
                    </div>
                </div>
                <div class="postbox newsarc-postbox-review">
                    <div class="postbox-header">
                        <h3 class="hndle ui-sortable-handle">
                            <?php esc_html_e('Leave us a Review', 'newsarc'); ?>
                        </h3>
                        <div class="hndle-controller">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                 fill="none">
                    ...
                    <div class="inside">
                        ...
                        <div><?php esc_html_e('Based on 20+ Reviews', 'newsarc'); ?></div>
                        <p>
                            <?php
                            echo sprintf(
                            /* translators: %s: Theme Name. */
                                esc_html__('What do you think of our theme? Was it a good experience and did it match your expectations? Let us know so we can improve!', 'newsarc'),
                                $theme->Name
                            );
                            ?>
                        </p>
                        <a href="<?php echo esc_url('https://wordpress.org/support/theme/newsarc/reviews/?rate=5#new-post'); ?>"
                           target="_blank"><?php esc_html_e('Submit a Review', 'newsarc'); ?></a>
                    </div>
                </div>

                <div class="postbox newsarc-postbox-support">
                    <div class="postbox-header">
                        <h3 class="hndle ui-sortable-handle">
                            <?php esc_html_e('Support', 'newsarc'); ?>
                        </h3>
                        <div class="hndle-controller">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                 fill="none">
                    ...
                    <div class="inside">
                        ...
                        <?php if (!$this->pro_status) { ?>
                            ...
                            <a href="<?php echo esc_url($this->community_url); ?>" target="_blank">
                                <?php esc_html_e('Community Support', 'newsarc'); ?>
                            </a>
                        <?php } else { ?>
                            <p>
                                <?php
                                echo sprintf(
                                /* translators: %s: Theme Name. */
                                    esc_html__('Need assistance? Reach out to our professional support team by submitting a support ticket, and we\'ll provide you with the help you need.', 'newsarc')
                                );
                                ?>
                            </p>
                            <a href="<?php echo esc_url($this->support_url); ?>" target="_blank">
                                <?php esc_html_e('Create a Ticket', 'newsarc'); ?>
                            </a>
                        <?php } ?>

                    </div>
                </div>
                <div class="postbox newsarc-postbox-plugins">
                    <div class="postbox-header">
                        <h3 class="hndle">
                            <?php esc_html_e('Useful Plugins', 'newsarc'); ?>
                        </h3>
                        <div class="hndle-controller">
                    ...
                    <div class="inside">
                        ...
                        $plugins = array(
                            array(
                                ...
                                'color' => '#5317AA',
                                'image' => NEWSARC_PARENT_URL . '/inc/admin/dashboard/images/ocdi-256x256.webp',
                            ...
                            array(
                                ...
                                'color' => '#5317AA',
                                'image' => NEWSARC_PARENT_URL . '/inc/admin/dashboard/images/addons-256x256.webp',
                        ...
                        foreach ($plugins as $plugin) {
                            ...
                            $is_plugin_installed = newsarc_is_plugin_installed($plugin_file);
                            ...
                            <div class="postbox-recommended-plugin">
                                ...
                                <?php if ($is_plugin_installed) : ?>
                                    <?php if ($is_plugin_activated) : ?>
                                        <div class="recommended-plugin-action"><?php esc_html_e('Activated', 'newsarc'); ?></div>
                                    <?php else : ?>
                                        <div class="recommended-plugin-action">
                                            <a href="#" class="activate-plugin"
                                               ...
                                               data-slug="<?php echo esc_attr($plugin_slug); ?> ">
                                                <?php esc_html_e('Activate', 'newsarc'); ?>
                                            </a>
                                        </div>
                                    <?php endif; ?>
                                <?php else : ?>
                                    <div class="recommended-plugin-action">
                                        <a href="#" class="install-plugin"
                                           data-plugin="<?php echo esc_attr($plugin_file); ?>"
                                           data-slug="<?php echo esc_attr($plugin_slug); ?> ">
                                            <?php esc_html_e('Install', 'newsarc'); ?>
                                        </a>
                                    </div>
                                <?php endif; ?>
                            </div>
                ...
                <div class="postbox newsarc-postbox-facebook">
                    <div class="postbox-header">
                        <h3 class="hndle">
                            <?php esc_html_e('Facebook Community', 'newsarc'); ?>
                        </h3>
                        <div class="hndle-controller">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                 fill="none">
                    ...
                    <div class="inside">
                        <p>
                            <?php echo esc_html__('Join our Facebook page to connect with like-minded individuals for discussions, help, and support on all things related to the theme!', 'newsarc'); ?>
                        </p>
                        <a href="<?php echo esc_url($this->facebook_url); ?>"
                           target="_blank"><?php esc_html_e('Like or follow our Facebook Page', 'newsarc'); ?></a>
                    </div>
                </div>
            </div>
        </div>
    </div><!--/.newsarc-dashboard-content-->
<?php
