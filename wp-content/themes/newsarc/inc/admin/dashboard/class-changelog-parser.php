<?php
/**
 * NewsArc Changelog Parser Class.
 *
 * @package NewsArc
 * @since   1.0.0
 */
...
/**
 * NewsArc Changelog Parser.
 */
class NewsArc_Changelog_Parser
{
    /**
     * Get item.
     *
    ...
    public function get_item($request)
    {
        ...
        if (false === $data) {
            return new \WP_Error('changelog_not_found', esc_html__('Changelog not found.', 'newsarc'));
        }
        $data = $changelog[$data];
    ...
    public function get_items_permissions_check()
    {
        if (!current_user_can('manage_themes')) {
            return new \WP_Error(
                'rest_forbidden',
                esc_html__('You are not allowed to access this resource.', 'newsarc'),
    ...
    protected function read_changelog()
    {
        ...
        if (!$wp_filesystem) {
            return new \WP_Error('filesystem_error', esc_html__('Could not access filesystem.', 'newsarc'));
        }
        ...
        if (!$raw_changelog) {
            return new \WP_Error('changelog_read_error', esc_html__('Failed to read changelog.', 'newsarc'));
        }
    ...
    public function prepare_item_for_response($item, $request)
    {
        ...
        return apply_filters('newsarc_blocks_prepare_changelog', $response, $item, $request);
    }
