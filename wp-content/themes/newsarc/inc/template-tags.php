 ...
 * @package NewsArc
 */
if (!function_exists('newsarc_posted_on')) :
    /**
     ...
     */
    function newsarc_posted_on($date_format = null, $date_label = null, $date_meta = null)
    {
        ...
        <div class="entry-meta entry-date posted-on">
            ...
            if ($date_meta == 'with_icon') :
                newsarc_the_theme_svg('calendar');
            endif;
            ...
            if ($date_format == 'time_ago') {
                ...
                /* translators: %s: post date. */
                    esc_html_x('%s', 'post date', 'newsarc'),
...
if (!function_exists('newsarc_posted_by')) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     ...
     */
    function newsarc_posted_by($author_meta = null, $meta_text = null)
    {
        // Author.
...
if (!function_exists('newsarc_post_category')) :
    /**
     ...
     * @since 1.0.0
     */
    function newsarc_post_category($show_color = null, $meta_text = null, $limit = 0)
    {
        ...
        <div class="entry-meta entry-categories cat-links<?php echo esc_attr($wrapper_class); ?>">
            ...
            if ($meta_text) : ?>
                <span class="entry-meta-label author-label"><?php echo esc_html($meta_text); ?></span>
            <?php else : ?>
                <span class="screen-reader-text"><?php esc_html_e('Posted in', 'newsarc'); ?></span>
            <?php endif;
            ?>
            <?php
            $style_attr = '';
...
if (!function_exists('newsarc_entry_footer')) :
    /**
     * Prints HTML with meta information for the categories, tags and comments.
     */
    function newsarc_entry_footer($show_tag = true, $show_comment = true)
    {
        // Hide category and tag text for pages.
        if ('post' === get_post_type() && $show_tag) {

            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'newsarc'));
            if ($tags_list) {
                /* translators: 1: list of tags. */
                printf('<span class="entry-meta entry-tags tags-links">' . esc_html__('Tagged %1$s', 'newsarc') . '</span>', $tags_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
        }
            ...
            comments_popup_link(
                sprintf(
                    ...
                    /* translators: %s: post title */
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'newsarc'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
        ...
        edit_post_link(
            sprintf(
                ...
                /* translators: %s: Name of current post. Only visible to screen readers */
                    __('Edit <span class="screen-reader-text">%s</span>', 'newsarc'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
...
if (!function_exists('newsarc_post_thumbnail')) :
    /**
     ...
     */
    function newsarc_post_thumbnail($image_class = null, $image_size = null)
    {
 ...
 *
 */
function newsarc_filter_wp_list_pages_item_classes($css_class, $page, $depth, $args)
{
...
add_filter('page_css_class', 'newsarc_filter_wp_list_pages_item_classes', 10, 4);
 ...
 *
 */
function newsarc_add_sub_toggles_to_main_menu($args, $item)
{
    ...
    if (isset($args->show_toggles) && $args->show_toggles) {
        ...
        if (in_array('menu-item-has-children', $item->classes, true)) {
            ...
            $toggle_duration = newsarc_toggle_duration();
            ...
            $args->after .= '<button class="toggle sub-menu-toggle" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint($toggle_duration) . '" aria-expanded="false"><span class="screen-reader-text">' .
                ...
                __('Show sub menu', 'newsarc') .
                '</span>' . newsarc_get_theme_svg('chevron-down') . '</button>';
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
            $args->link_after = '<span class="icon">' . newsarc_get_theme_svg( 'chevron-down' ) . '</span>';
        } else {
            $args->link_after = '';
        }
	}
...
add_filter('nav_menu_item_args', 'newsarc_add_sub_toggles_to_main_menu', 10, 2);
 ...
 */
function newsarc_show_main_menu_nav_description( $item_output, $item, $depth, $args ) {
    if ( ! empty( $item->description ) ) {
        $item_output = str_replace( $args->link_after . '</a>', '<span class="menu-item-description">' . $item->description . '</span>' . $args->link_after . '</a>', $item_output );
    }
    return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'newsarc_show_main_menu_nav_description', 10, 6 );
 ...
 *
 */
function newsarc_nav_menu_social_icons($item_output, $item, $depth, $args)
{
    // Change SVG icon inside social links menu if there is supported URL.
    if ('social' === $args->theme_location) {
        $svg = NewsArc_SVG_Icons::get_social_link_svg($item->url);
        if (empty($svg)) {
            $svg = newsarc_get_theme_svg('link');
        }
...
add_filter('walker_nav_menu_start_el', 'newsarc_nav_menu_social_icons', 10, 4);
 ...
 *
 */
function newsarc_toggle_duration()
{
    /**
     ...
     *
     */
    $duration = apply_filters('newsarc_toggle_duration', 250);
