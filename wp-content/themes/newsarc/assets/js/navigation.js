...
// event "polyfill"
newsarc.createEvent = function (eventName) {
    var event;
    ...
    hideAndShowModals: function () {
        ...
        modals.forEach(function (modal) {
            modal.addEventListener('toggle-target-before-inactive', function (event) {
                ...
                _win.newsarc.scrolled = parseInt(styles.top, 10);
                if (adminBar) {
            ...
            modal.addEventListener('toggle-target-after-inactive', function (event) {
                ...
                setTimeout(function () {
                    var clickedEl = newsarc.toggles.clickedEl;
                    ...
                    _win.scrollTo(0, Math.abs(_win.newsarc.scrolled + getAdminBarHeight()));
                    _win.newsarc.scrolled = 0;
                }, 500);
            });
        });
    },
...
newsarc.modalMenu = {
    ...
    expandLevel: function () {
        ...
        modalMenus.forEach(function (modalMenu) {
            ...
            if (activeMenuItem) {
                newsarcFindParents(activeMenuItem, 'li').forEach(function (element) {
                    var subMenuToggle = element.querySelector('.sub-menu-toggle');
                    if (subMenuToggle) {
                        newsarc.toggles.performToggle(subMenuToggle, true);
                    }
                });
            }
        });
    ...
    keepFocusInModal: function () {
        ...
        _doc.addEventListener('keydown', function (event) {
            var toggleTarget, modal, selectors, elements, menuType, bottomMenu, activeEl, lastEl, firstEl, tabKey,
                shiftKey, clickedEl = newsarc.toggles.clickedEl;
...
newsarc.toggles = {
    ...
    performToggle: function (element, instantly) {
        ...
        if (target.classList.contains(activeClass)) {
            target.dispatchEvent(newsarc.createEvent('toggle-target-before-active'));
        } else {
            target.dispatchEvent(newsarc.createEvent('toggle-target-before-inactive'));
        }
        ...
        setTimeout(function () {
            ...
            if (toggle.dataset.toggleType === 'slidetoggle' && !instantly && duration !== '0') {
                newsarcMenuToggle(newTarget, duration);
            } else {
            ...
            newsarcToggleAttribute(toggle, 'aria-expanded', 'true', 'false');
            if (self.clickedEl && -1 !== toggle.getAttribute('class').indexOf('close-')) {
                newsarcToggleAttribute(self.clickedEl, 'aria-expanded', 'true', 'false');
            }
            // Toggle body class.
            ...
            target.dispatchEvent(newsarc.createEvent('toggled'));
            // Trigger events on the toggle targets after they are toggled.
            if (target.classList.contains(activeClass)) {
                target.dispatchEvent(newsarc.createEvent('toggle-target-after-active'));
            } else {
                target.dispatchEvent(newsarc.createEvent('toggle-target-after-inactive'));
            }
        }, timeOutTime);
    },
    // Do the toggle.
...
newsarcDomReady(function () {
    newsarc.toggles.init();              // Handle toggles.
    newsarc.coverModals.init();          // Handle cover modals.
    newsarc.modalMenu.init();            // Modal Menu.
    newsarc.primaryMenu.init();          // Primary Menu.
    newsarc.touchEnabled.init();         // Add class to body if device is touch-enabled.
});
/*	-----------------------------------------------------------------------------------------------
	Helper functions
--------------------------------------------------------------------------------------------------- */
...
/* Toggle an attribute ----------------------- */
function newsarcToggleAttribute(element, attribute, trueVal, falseVal) {
    var toggles;
 ...
 */
function newsarcMenuToggle(target, duration) {
 ...
 */
function newsarcFindParents(target, query) {
    var parents = [];
